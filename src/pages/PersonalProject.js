    ...
    return (
        <div className="project">
            <div className="headerBar">
                <NavigateBeforeIcon 
                    onClick={() => {
                        Number(id) === 0 ? navigate("/professional_project/" + (ProfessionalProjectList.length -1))
                            : navigate("/personal_project/" + (Number(id) - 1))
                    }} 
                />
                <h1>{project.name}</h1>
                <NavigateNextIcon 
                    onClick={() => {
                        Number(id) >= (PersonalProjectList.length - 1) ?
                            navigate("/professional_project/" + 0)
                            : navigate("/personal_project/" + (Number(id) + 1)) 
                    }}
                />
            </div>
